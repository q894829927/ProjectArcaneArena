...
| G-C：三种攻击模式 | Partial | 单发与散射已通过 PIE；直线 Pierce 源码已实现 PierceCount+1 命中上限、同帧沿线多目标排序和每 Projectile 目标去重 | 待 UBT/PIE 验证 Pierce 与 Spread+Pierce 组合 |
...
| P4：Spread / Pierce / 多武器 | Partial | 多武器与 Spread/PelletFalloff 已通过 PIE；直线 Pierce 源码已接入 | 待 Pierce UBT/PIE；通过后 P4/G-C 可提升为 Implemented |
...
当前实现进度：P0/P1 为 `Partial`；P2 / G-A、P3、G-B 与 P4-C Spread 为 `Implemented`。P4-D 直线 Pierce 已完成源码接线但尚未本地验收，因此 G-C / P4 整体仍为 `Partial`。P5～P7 与 G-D～G-F 仍为 `Planned`。
