...
| P4：Spread / Pierce / 多武器 | Partial | WeaponDataAsset、WeaponLoadoutComponent、多 Runtime 独立调度均已接入并通过双槽 PIE | 多武器基础已完成；下一步实现 Spread 与真正 Pierce 去重 |
...
当前实现进度：P0/P1 为 `Partial`；P2 / G-A 与 P3 为 `Implemented`；G-B 多槽 WeaponRuntime 已通过双槽 PIE，状态为 `Implemented`。P4 整体仍为 `Partial`，因为 Spread/Pierce 尚未实现；P5～P7 与 G-C～G-F 仍为 `Planned`。
