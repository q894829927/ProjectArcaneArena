...
* During `GA_Dash`, the predicted client and authority temporarily change the player capsule's `Pawn` response to `Overlap`, allowing the dash to pass through enemies and other Pawns while preserving world collision. Every normal, cancelled, stunned, dead, or rejected end path restores the saved response in `EndAbility()`.
