...
* The trail lasts `2s`, uses a `120`-unit strict two-dimensional point-to-segment radius, and applies base `6` Lightning damage immediately and every `0.5s`. Each target is settled at most once per tick through `GE_Damage`, preserving AttackPower, Crit, Defense, Shield-first, Shocked vulnerability, hit Cues, OnCrit, OnKill, and optional Overload synergy.
...
* Successful damage emits one type-specific hit Cue plus one normal-or-critical number Cue from the authoritative AttributeSet Damage meta-attribute path, using actual Shield plus Health loss and HitResult/target-location fallback. `UArenaAbilitySystemComponent` preserves every settlement as an independent parameter item but sends all damage feedback queued for the same target and Tick through one unreliable batch Multicast, preventing concurrent direct, periodic, and synergy hits from exhausting the default two-RPC-per-net-update limit.
...
* Native tags, the eight-second cooldown GE and `AArenaGameplayCueNotify_BossFireZoneRadius` support dynamically scaled Telegraph and Active Niagara. `Content/Python/boss/setup_boss_fire_zone.py` provides idempotent post-build creation for animation/Montage, GA/GE, Area Blueprint, two Boss-local Niagara/Cue assets and one Boss StartupAbilities entry without modifying the Behavior Tree graph.
* The editor setup run completed without Python errors and saved the animation, Montage, both Niagara systems, GA, cooldown, Area, both Cue Blueprints and updated Boss StartupAbilities. C++/UHT build, manual `GroundSlam -> Charge -> FireZone -> Chase -> Wait` wiring, runtime authority/cleanup tests, overlapping-area independence and dual-view VFX readability remain pending.
