...
* Native tags, the eight-second cooldown GE and `AArenaGameplayCueNotify_BossFireZoneRadius` support dynamically scaled Telegraph and Active Niagara. The Active Cue now combines its fire effect with a boundary Niagara and an always-visible ring mesh scaled from the authoritative radius, so the damage boundary no longer depends on a one-shot particle lifetime. `Content/Python/boss/setup_boss_fire_zone.py` provides idempotent post-build creation for animation/Montage, GA/GE, Area Blueprint, two Boss-local Niagara/Cue assets and one Boss StartupAbilities entry without modifying the Behavior Tree graph.
