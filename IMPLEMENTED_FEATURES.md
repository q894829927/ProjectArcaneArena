...
* The Charge branch is manually wired and its single-player core behavior is partially verified: the `350-900` band, locked dodgeable path, one-hit-per-player guard, wall/endpoint completion, cooldown fallback and normal cleanup work. A follow-up source change ignores walkable-floor collision hits and enlarges/raises the telegraph; slope traversal and the revised warning still require recompilation and PIE verification.
