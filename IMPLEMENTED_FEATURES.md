...
* Boss AI now has a configurable `InitialAbilityDelay` of `1.5s`. The Behavior Tree may acquire a target and Chase immediately, while every GAS Ability branch remains ineligible until the authority-side opening buffer expires; this does not consume or fabricate an Ability cooldown.
