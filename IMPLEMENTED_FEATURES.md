...
* `UArenaItemDataAsset` provides a unique item tag, multi-value filter tags, display data, max stack size, configurable recovery GameplayEffect/SetByCaller magnitude, shared cooldown class and world Pickup class.
* `AArenaInventoryPickupActor` is a replicated, server-consumed interaction Pickup separate from the existing instant recovery drops. Its mesh rotates independently while the name/quantity label faces each local camera and uses distinct Health/Energy colors. `G` submits the nearest visible candidate within `220` units, while the server rechecks phase, state, distance, visibility, protection time and Actor lifetime.
...
* `ArenaInventory::CalculatePageCount()` and `MatchesFilters()` are the shared rule path used by the Controller and two development Automation Tests. The tests encode the required `0/19/20/21/40/41` page boundaries plus empty, OR, parent-tag, unique-item-tag, and unrelated-filter behavior; they are implemented but await execution in the newly compiled Editor target.
* `Tab` opens the inventory only during Combat, stops Sprint and locks local movement/look/active-skill input without pausing the world or copying UI state into GameplayTags. Closing restores the previous top-down or third-person mouse/reticle mode.
* `Content/Python/inventory/setup_inventory_items.py` has created the tunable shared cooldown GE, `DA_HealthPotion`, `DA_EnergyPotion`, their placeable Pickup Blueprints, `WBP_Inventory`, and `WBP_InventorySlot`. It connected the slot class and `/Game/Core/BP_ArenaPlayerController.InventoryWidgetClass`; the 2026-07-27 post-save run reloaded and verified all eight gameplay/UI assets and found no `_1/_2` duplicates. Automation, PIE and OwnerOnly multiplayer verification remain pending.
