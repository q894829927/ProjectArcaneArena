...
- `/Game/ProjectArcaneArena/Systems/Pickups/Blueprints/BP_HealthPickup`：恢复 `25 Health`，`15s` 后超时销毁。
- `/Game/ProjectArcaneArena/Systems/Pickups/Blueprints/BP_EnergyPickup`：恢复 `20 Energy`，`15s` 后超时销毁。
- `/Game/ProjectArcaneArena/Systems/Pickups/Data/DA_PickupDropTable_Default`：`25%` 掉率，Health/Energy 权重均为 `1`。
- `/Game/ProjectArcaneArena/Core/GameMode/BP_ArenaGameMode`：将 `PickupDropTable` 设置为上述默认表。
