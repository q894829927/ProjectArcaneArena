...
1. 在顶视角和第三人称分别按 `Escape` 打开菜单，验证继续、设置占位、返回主菜单和退出游戏按钮均可直接点击；再次按 `Escape` 应关闭菜单。
2. 菜单打开时验证当前玩家停止 Sprint、移动、转镜头、切换视角、拾取和释放主动技能，关闭后鼠标位置、准星、移动和技能输入按原视角恢复且没有残留输入锁。
3. 在 Upgrade、Inventory、BossIntro、BossOutro 和 Victory 前后打开或关闭菜单，确认 Upgrade 候选能恢复、演出和 Victory 焦点不冲突，阶段切换会安全关闭旧菜单。
4. Listen Server 下分别由 Host 和 Client 打开菜单，确认只锁当前本地玩家且服务器和其他玩家继续运行；Client 返回主菜单只离开自己，Host 返回主菜单会让全部客户端回到菜单。
5. 在 PIE、Standalone 和打包版验证退出游戏；点击设置只显示“设置功能暂未开放”，不会修改输入、画面或玩法配置。

### 通过标准

- 菜单可以用鼠标和键盘稳定打开、操作和关闭，点击背景后按钮仍可点击，重复快速开关不会叠加移动/观察锁或丢失焦点。
- 返回主菜单和退出不会残留 Pawn 输入锁、背包/升级 Widget、鼠标捕获、第三人称准星或 Direct IP 连接状态。
...
1. 进入正式关卡后确认左上角默认显示 `FPS` 与 `Ping`，并且约每 `0.25s` 平稳更新而不是逐帧闪烁。
2. 连续按 `P` 确认只切换本地统计叠层，不影响移动、技能、顶视角/第三人称切换或其他客户端。
3. Listen Server 下确认 Host 显示接近 `0 ms`，远端 Client 显示由 `APlayerState` 维护的非零往返延迟。
4. 在 Upgrade、Inventory、BossIntro/Outro 和 Victory 输入模式前后复测开关，确认 HUD 与输入焦点不会残留异常。

### 通过标准

- 两种视角与 Host/Client 都能独立显示和关闭统计；数值只属于本地表现，不产生 RPC、复制字段或服务器玩法变化。
- 720p 与 1080p 下左上角文本不裁切、不遮挡关键 HUD，打包版 `P` 输入与 PIE 一致。
...
4. 分别在顶视角和第三人称检查预警范围、Active 火焰、持续伤害边界圈和移动躲避可读性。

### 通过标准

- 客户端不运行伤害 Timer；所有 `GE_Damage + Damage.Fire` 只由服务器应用并在两端得到一致 Shield/Health 结果。
- Boss 开场缓冲只阻断服务器行为树技能分支，不阻塞选敌或 Chase，也不添加虚假的 Cooldown Tag。
...
双人首次显示 `2100/2100`、两名玩家完成 `PostLogin` 后才启动 Boss 波的核心路径已于 2026-07-25 验证通过。

1. 在 Boss 生成前让一名已连接玩家死亡或暂时缺少 Pawn，确认该 `AArenaPlayerState` 仍计入快照并得到 `2100`。
2. Boss 生成后分别让玩家死亡、复活、掉线或重新拥有 Pawn，确认已经确定的 MaxHealth 不重新缩放。
3. 确认 Phase 2 在 `1470`、Phase 3 在 `735` 附近触发；两端阶段 Tag、HUD、Enrage 属性和 Cue 一致。
...
3. [已完成] 将 `ProjectileSpeed` 提高到 10000 后，AttackID 16～21 仍连续产生 `DataProjectile hit` 与实际伤害日志，未观察到单帧跨越 Capsule 的漏命中。
4. [已完成] 同一直线存在多个敌人时，非穿透 Projectile 只命中沿路径最早的一个目标并立即回收；真正 Pierce 规则留到 P4。
5. 杀死敌人后确认其不再进入 SpatialGrid 有效 Cell；销毁/换波后 WeakObjectPtr 和主动 Unregister 均无残留命中。
6. [暂缓] 两人 Listen Server 的 Source ASC、Damage/Crit/OnKill 归属专项验证暂不执行；保留为后续网络回归项。
...
- [已完成] 高速直线弹在 `ProjectileSpeed=10000` 下连续命中，未观察到因单帧位移跨过敌人而漏判。
- [已完成] 非穿透弹一发最多结算一个最早目标；槽位释放后 Generation 正常递增。
